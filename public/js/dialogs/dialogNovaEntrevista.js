function dialogNovaEntrevista(url, url_entrevista ,csrf) {
    $('#dialog-nova-entrevista').hide();
    $('#btnCadEntrevista').click(function () {
        $("#dialog-nova-entrevista").dialog({
            resizable: false,
            height: "auto",
            width: 800,
            modal: true,
            create: function () {
                var table = $('#table-entrevista').DataTable({
                    processing: true,
                ...
                $('#table-entrevista tbody').on('click', 'td.btnShowCurriculo', function () {
                    var tr = $(this).closest('tr');
                    var row = table.row(tr);
                    window.location.href =  url_entrevista + "?id=" + row.data().id;
                });
            },
            buttons: {
                Cancelar: function () {
